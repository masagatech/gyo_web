...
export class AddLocationComponent implements OnInit, AfterViewInit {
    ...
    fillStateDropDown() {
        ...
        that._locservice.getLocationDetails({ "flag": "dropdown" }).subscribe(data => {
            try {
                ...
                setTimeout(function () { $('#ddlstate').selectpicker('refresh'); }, 100);
            }
            catch (e) {
    ...
    private onStateChange(e) {
        if (this.stateid > 0) {
            ...
            this.ctid = 0;
            this.arid = 0;
            setTimeout(function () { $('#ddlcity').selectpicker('refresh'); }, 100);
        }
    }
    ...
    private onCityChange(e) {
        ...
        if (this.ctid > 0) {
            ...
            setTimeout(function () { $('#ddlarea').selectpicker('refresh'); }, 100);
        }
    }

...
export class BatchReportsComponent implements OnInit, OnDestroy {
    ...
    public exportToCSV() {
        new Angular2Csv(this.batchDT, 'User Details', { "showLabels": true });
    }

    public exportToPDF() {
        let doc = new jsPDF();
        doc.text(20, 20, JSON.stringify(this.batchDT));
        doc.save('Test.pdf');
    ...
    public viewBatchDataRights() {
        ...
        that._menuservice.getMenuDetails({
            "flag": "actrights", "uid": that.loginUser.uid, "ucode": that.loginUser.ucode, "mcode": "rptbatch", "utype": that.loginUser.utype
        }).subscribe(data => {
